...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for band in orm.Band.objects.all():
            if orm.BandMember.objects.filter(band=band).count() == 0:
                new_member = orm.BandMember()
                new_member.user = orm.Profile.objects.filter(solo_band=band)[0].user
                new_member.band = band
                new_member.role = 0
                new_member.save()
